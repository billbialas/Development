...
{
  string uploadingFolder = "~/Editor/SWF/users_flashes/";

  Page.Response.Write("<html>");
  Page.Response.Write("<head>");
  Page.Response.Write("<scr"+"ipt>");
  Page.Response.Write("var flashSaved = \""+Page.Request["notsaved"]+"\";");
  Page.Response.Write("var flashFileName = null;");
  Page.Response.Write("var flashFileTitle = null;");
  ...
  if(Page.Request.Files.Count > 0)
  {
    ...
    if(PFile.FileName.Trim().Length > 0 && PFile.ContentLength > 0)
    {
      ...
      try
      {
         ...
         Page.Response.Write("flashSaved = \""+Page.Request["saved"]+"\";");
         Page.Response.Write("flashFileName = \""+ResolveUrl(uploadingFolder)+Path.GetFileName(PFile.FileName)+"\";");
         Page.Response.Write("flashFileTitle = \""+title+"\";");
      }
      catch(Exception ev)
      {
         Page.Response.Write("flashSaved = \""+ev.Message.Replace("\n"," ").Replace("\r"," ").Replace("\\","\\\\").Replace("\"","\\\"")+"\\n\\nTurn to your System Administrator.\";");
      }
    }
  }
