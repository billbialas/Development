...
//"http://dww.webmagicportal.com/myImageBrowseServer.aspx";
	string externalImagesGalleryBrowserA = System.Configuration.ConfigurationManager.AppSettings["CurrentIMGURL"] + "/myImageBrowseServer.aspx";

	string externalImagesGalleryBrowser = System.Configuration.ConfigurationManager.AppSettings["CurrentIMGURL"] + "/myImageBrowseServer.aspx?id=" ;
...
void Page_Load(object sender, EventArgs e)
{
  ...
  try
  {
  		String strCookieName = "bb";
 	 	HttpCookie cookieA = Request.Cookies[strCookieName];
   	String strCookieValue = cookieA.Value.ToString();

  	string externalImagesGalleryBrowserX =externalImagesGalleryBrowser + strCookieValue;
    txtReader = new XmlTextReader(externalImagesGalleryBrowserX );
...
function myDive(el,seek)
{
 if(el.firstChild && el.firstChild.tagName && el.firstChild.tagName.toLowerCase()=="span")
 {
  ...
  if(href.length > 0)
  {
   ...
   {
     document.getElementById("previewFrame").src = "<%= externalImagesGalleryBrowserA %>?imgsrc="+el.firstChild.getAttribute("myAttrRel");
     document.getElementById("previewFrameText").src = "<%= externalImagesGalleryBrowserA %>?imgprop="+el.firstChild.getAttribute("myAttrRel");
     document.getElementById("previewFrameTitle").src = "<%= externalImagesGalleryBrowserA %>?imgtitle="+el.firstChild.getAttribute("myAttrRel");
     ob_t25(el);
