...
void Page_Load(object sender, EventArgs e)
{
        ...
        string folder = System.Web.HttpContext.Current.Server.MapPath(ResolveUrl(flashFolder));

        if(Page.Request["flashsrc"] != null)
        if(Page.Request["flashsrc"].Length > 0)
        {
          string flashFile = System.Web.HttpContext.Current.Server.MapPath(Page.Request["flashsrc"].Replace("%20"," "));

          if(File.Exists(flashFile))
          {
            string cont="<embed wmode=\"transparent\" src="+Page.Request["flashsrc"].Replace("%20"," ")+" width="+maxWidth.ToString()+" height="+maxHeight.ToString()+" quality=\"low\" pluginspage=\"http://www.macromedia.com/go/getflashplayer\" >";
        ...
        {
          oTree.Add ("root", "myRoot", "Flashes Root", true, null, null);
          string currentPath = ResolveUrl(Path.GetDirectoryName(Page.Request.ServerVariables["SCRIPT_NAME"])).Replace("\\","/")+"/";
          string swfPath = ResolveUrl(flashFolder);
          string differ = swfPath.Replace(currentPath,"");
          directoryDive(oTree, "myRoot", folder, curPath+ResolveUrl(flashFolder)+"/",differ+"/");
        }
...
function myDive(el,seek)
{
 ...
 if(el.firstChild && el.firstChild.tagName && el.firstChild.tagName.toLowerCase()=="span")
 {
  ...
  if(found)
  {
    document.getElementById("previewFrame").src = window.location.href+"&flashsrc="+el.firstChild.getAttribute("myAttrRel");
    ob_t25(el);
    return true;
  }
 }
...
//
// Important. Change the flash URL in parent window.
//--------------------------------------------------------
  try
  {
